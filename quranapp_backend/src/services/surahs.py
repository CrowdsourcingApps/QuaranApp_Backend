...
def get_surahs_in_mushaf(db: Session, mushaf_id: uuid.UUID) -> list[SurahInMushaf]:
    return (db.query(SurahInMushaf).join(MushafPage, SurahInMushaf.first_page_id == MushafPage.id)
            .filter(MushafPage.mushaf_id == mushaf_id).all())


def get_surahs_in_mushaf_mapped(db: Session, mushaf_id: uuid.UUID):
